...
| Trường hợp | HTTP Status | Ghi chú |
|---|---|---|
| fromDate > toDate | 400 Bad Request | Validate trong Service trước khi query |
| fromDate hoặc toDate null/sai format | 400 Bad Request | @Valid + @DateTimeFormat |
| Export type không hợp lệ (không phải revenue/inventory/staff) | 400 Bad Request | Enum validation |
| Export format không hợp lệ (không phải excel/pdf) | 400 Bad Request | Enum validation |
| cron expression không hợp lệ khi tạo lịch | 400 Bad Request | Validate bằng CronExpression.isValidExpression() |
| JWT token không hợp lệ / hết hạn | 401 Unauthorized | Xử lý ở API Gateway / Security Filter |
| Không đủ quyền (không phải ADMIN/BRANCH_MANAGER) | 403 Forbidden | Spring Security @PreAuthorize |
| Branch Manager truy cập report của chi nhánh khác | 403 Forbidden | Kiểm tra X-Branch-Id header |
| Không có dữ liệu trong kỳ | 200 OK | data = [] hoặc totalRevenue = 0 |
| Feign Client timeout / service down (order-service, inventory-service) | 503 Service Unavailable | @FeignClient fallback hoặc CircuitBreaker |
| Lỗi sinh file Excel/PDF (out of memory, corrupt data) | 500 Internal Server Error | Try-catch trong ExcelExportServiceImpl / PdfExportServiceImpl |
| Lỗi runtime không mong đợi | 500 Internal Server Error | GlobalExceptionHandler bắt Exception.class |
...
| Trường hợp | HTTP Status | Ghi chú |
|---|---|---|
| Keyword rỗng / null | 200 OK | Trả về danh sách ACTIVE toàn bộ (không báo lỗi) |
| minPrice > maxPrice | 400 Bad Request | Validate trong SearchServiceImpl trước khi query |
| minPrice hoặc maxPrice âm | 400 Bad Request | @Min(0) trên query param |
| page < 0 hoặc size <= 0 | 400 Bad Request | Spring Pageable validation |
| medicineId không tồn tại (GET/PUT/DELETE /medicines/{id}) | 404 Not Found | ResourceNotFoundException |
| CategoryId không tồn tại | 404 Not Found | Kiểm tra qua CategoryClient hoặc DB |
| JWT token không hợp lệ / hết hạn | 401 Unauthorized | Xử lý ở API Gateway / Security Filter |
| Tạo/sửa/xóa thuốc không đủ quyền (không phải ADMIN/WAREHOUSE_STAFF) | 403 Forbidden | Spring Security @PreAuthorize |
| Không có kết quả tìm kiếm | 200 OK | data = [], totalElements = 0 |
| Feign Client timeout / service down (InventoryClient, CategoryClient, SupplierClient) | 503 Service Unavailable | Fallback trả về inStock = null, không block search |
| Lỗi runtime không mong đợi | 500 Internal Server Error | GlobalExceptionHandler bắt Exception.class |
...
| Trường hợp | HTTP Status | Ghi chú |
|---|---|---|
| Thiếu field bắt buộc (name, phone, tax_code) khi tạo NCC | 400 Bad Request | @Valid + @NotBlank trên CreateSupplierRequest |
| tax_code sai format (không phải số, quá dài/ngắn) | 400 Bad Request | @Pattern validation |
| phone sai format | 400 Bad Request | @Pattern validation |
| email sai format | 400 Bad Request | @Email validation |
| JWT token không hợp lệ / hết hạn | 401 Unauthorized | Xử lý ở API Gateway / Security Filter |
| Thao tác write (POST/PUT/DELETE) không phải ADMIN | 403 Forbidden | Spring Security @PreAuthorize |
| GET / DELETE NCC không tồn tại | 404 Not Found | ResourceNotFoundException |
| Tạo NCC với tax_code đã tồn tại | 409 Conflict | DataIntegrityViolationException từ DB Unique Constraint |
| Feign Client timeout / service down (khi lấy supply history từ inventory-service) | 503 Service Unavailable | Fallback trả về history = [] |
| Lỗi runtime không mong đợi | 500 Internal Server Error | GlobalExceptionHandler bắt Exception.class |
...
| Trường hợp | HTTP Status | Ghi chú |
|---|---|---|
| Thiếu field bắt buộc (patientId, diagnosis, items[]) khi tạo đơn | 400 Bad Request | @Valid + @NotNull/@NotEmpty trên CreatePrescriptionRequest |
| items[] rỗng khi tạo đơn | 400 Bad Request | @NotEmpty validation trên danh sách thuốc |
| quantity <= 0 trong PrescriptionItemRequest | 400 Bad Request | @Min(1) validation |
| orderId không hợp lệ khi link-order | 400 Bad Request | Validate UUID format |
| JWT token không hợp lệ / hết hạn | 401 Unauthorized | Xử lý ở API Gateway / Security Filter |
| Không phải PHARMACIST tạo/ký/hủy đơn | 403 Forbidden | Spring Security @PreAuthorize("hasRole('PHARMACIST')") |
| SALES_STAFF truy cập endpoint chỉ dành cho PHARMACIST | 403 Forbidden | Spring Security role check |
| patientId không tồn tại | 404 Not Found | FeignException từ CustomerClient → map sang 404 |
| Đơn thuốc {id} không tồn tại | 404 Not Found | ResourceNotFoundException |
| Mã code {code} không tồn tại | 404 Not Found | ResourceNotFoundException |
| Chỉnh sửa đơn đã SIGNED hoặc CANCELLED | 409 Conflict | Service kiểm tra status trước update |
| Ký đơn đã SIGNED hoặc CANCELLED | 409 Conflict | State machine check |
| Hủy đơn đã liên kết với đơn hàng (orderId != null) | 409 Conflict | Service kiểm tra orderId trước cancel |
| Mã code bị trùng (race condition) | Retry nội bộ | generateCode() tự retry tối đa 3 lần |
| Feign Client timeout / service down (CustomerClient, UserClient) | 503 Service Unavailable | FeignException → map sang 503 |
| Lỗi runtime không mong đợi | 500 Internal Server Error | GlobalExceptionHandler bắt Exception.class |
...
| Trường hợp | HTTP Status | Ghi chú |
|---|---|---|
| Thiếu field bắt buộc (recipientId, title, body) | 400 Bad Request | @Valid + @NotBlank trên CreateNotificationRequest |
| Bulk notification với recipientIds[] rỗng | 400 Bad Request | @NotEmpty validation |
| Broadcast với roles[] và branchId đều null | 400 Bad Request | Custom validator |
| Template key không tồn tại trong DB | 400 Bad Request | TemplateResolver ném TemplateNotFoundException → 400 |
| channel không hợp lệ (không phải IN_APP/SMS/EMAIL) | 400 Bad Request | Enum deserialization validation |
| JWT token không hợp lệ / hết hạn | 401 Unauthorized | Xử lý ở API Gateway / Security Filter |
| Xóa thông báo của người khác (X-User-Id != recipient_id) | 403 Forbidden | Service kiểm tra ownership trước softDelete |
| Gửi/broadcast thông báo không phải ADMIN | 403 Forbidden | Spring Security @PreAuthorize |
| Notification {id} không tồn tại | 404 Not Found | ResourceNotFoundException |
| recipient_id không tồn tại (user không có trong hệ thống) | 404 Not Found | Validate trước hoặc nhận FeignException |
| Retry thông báo đã SENT hoặc READ | 409 Conflict | Service kiểm tra status trước retry |
| Outbox Consumer xử lý thất bại sau max retry | 500 Internal Server Error | Ghi log + cập nhật outbox_logs.status = FAILED |
| Lỗi runtime không mong đợi | 500 Internal Server Error | GlobalExceptionHandler bắt Exception.class |
