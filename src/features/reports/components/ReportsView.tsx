...
export function ReportsView() {
	const [reportType, setReportType] = useState<"revenue" | "inventory">(
		"revenue",
	);
	...
	const exportPayload: ReportPayload | null = useMemo(() => {
		...
		if (reportType === "revenue") {
			...
			const totalGross = rows.reduce(
				(s: number, r: any) => s + (r.gross || 0),
				0,
			);
			const totalDiscount = rows.reduce(
				(s: number, r: any) => s + (r.discount || 0),
				0,
			);
			...
			return {
				...
				columns: [
					...
					},
					{
						key: "gross",
						header: "Tổng bán",
						format: (v) => formatVND(v as number),
					},
					{
						key: "discount",
						header: "Giảm giá",
		...
		<DashboardLayout>
			<div className="mb-6">
				...
				<p className="text-sm text-ink-500 mt-1">
					UC09 - FR9.1-FR9.5 · Báo cáo doanh thu, tồn kho, xuất Excel/PDF
				</p>
			</div>
			...
			<Card className="mb-4">
				<div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
					...
					{reportType === "inventory" && (
						<div className="sm:col-span-3 self-end text-xs text-ink-500">
				...
				<>
					...
					<Card title={`Chi tiết doanh thu (${(data.rows || []).length} dòng)`}>
						{!data.rows || data.rows.length === 0 ? (
							<p className="text-center text-ink-500 py-6">
								Không có dữ liệu trong khoảng thời gian này (MSG25)
							</p>
						) : (
			...
			{data && reportType === "inventory" && (
				<Card
					title={`Tồn kho: ${data.totalBatches} lô, ${data.lowStockCount} cảnh báo`}
				>
					{data.lowStockCount > 0 && (
						<Alert
							variant="warning"
							title={`${data.lowStockCount} lô sắp hết hàng`}
							className="mb-3"
						>
							Cần nhập thêm hàng
						</Alert>
					...
					<p className="text-sm text-ink-600">
						Tổng số lô hàng trong hệ thống: <strong>{data.totalBatches}</strong>
					</p>
				</Card>
			)}
