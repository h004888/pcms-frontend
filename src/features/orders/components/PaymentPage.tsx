...
export default function PaymentPage() {
  ...
  return (
    <DashboardLayout>
      ...
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-ink-900">Thanh toán</h1>
        <p className="text-sm text-ink-500 mt-1">SCR-PAYMENT · Đơn hàng: {order.orderNumber}</p>
      </div>

      {paidPayment ? (
        <Card>
          <div className="text-center py-6">
            <div className="inline-flex p-4 bg-accent-50 rounded-full mb-3">
              <CreditCard className="w-10 h-10 text-accent-600" />
            </div>
            <h2 className="text-2xl font-bold text-accent-700 mb-1">Thanh toán thành công!</h2>
            <p className="text-sm text-ink-500">Mã hóa đơn: <strong>{paidPayment.invoiceNumber || 'Đang tạo...'}</strong></p>
            <div className="mt-4 max-w-sm mx-auto bg-ink-50 rounded-lg p-4 text-left">
              ...
              {paidPayment.changeAmount && paidPayment.changeAmount > 0 && (
                <div className="flex justify-between text-sm text-accent-700"><span>Tiền thừa:</span><strong>{formatVND(paidPayment.changeAmount)}</strong></div>
              )}
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <Card title="Thông tin thanh toán">
            <div className="space-y-3 text-sm">
              <div className="flex justify-between"><span className="text-ink-500">Mã đơn:</span><span className="font-mono">{order.orderNumber}</span></div>
              <div className="flex justify-between"><span className="text-ink-500">Số sản phẩm:</span><span>{order.items.length}</span></div>
              <div className="flex justify-between"><span className="text-ink-500">Tạm tính:</span><span>{formatVND(order.subtotal)}</span></div>
              <div className="flex justify-between"><span className="text-ink-500">Giảm giá:</span><span className="text-red-600">-{formatVND(order.discount)}</span></div>
              <div className="border-t pt-3 flex justify-between text-xl font-bold">
                <span>Tổng:</span><span className="text-accent-700">{formatVND(order.total)}</span>
              </div>
            </div>
          </Card>
          ...
          <Card title="Phương thức thanh toán">
            ...
            {method === 'CASH' ? (
              <>
                ...
                />
                {tenderedAmount > 0 && (
                  <div className="bg-accent-50 border border-accent-200 rounded-md p-3 text-sm">
                    ...
                    <div className="flex justify-between text-accent-700 font-bold">
