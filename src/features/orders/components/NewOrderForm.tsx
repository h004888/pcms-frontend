// =====================================================
// PCMS - New Order Form (multi-step cart)
// =====================================================
...
import { Search, Trash2, Save, ArrowLeft, Pill } from 'lucide-react';
...
export function NewOrderForm() {
  ...
  return (
    <DashboardLayout>
      ...
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-ink-900">Tạo đơn hàng mới</h1>
        <p className="text-sm text-ink-500 mt-1">SCR-ORDER-NEW · BR04: 5% discount tự động cho qty ≥ 10</p>
      </div>
          ...
          <Card title="2. Tìm & thêm thuốc" subtitle="Gõ tên hoặc SKU để tìm">
            <div className="relative">
              ...
              {medicineResults.length > 0 && (
                <div className="absolute z-10 mt-1 w-full bg-white border border-ink-200 rounded-md shadow-lg max-h-60 overflow-y-auto">
                  {medicineResults.map((m) => (
                    <button
                      key={m.id}
                      onClick={() => addToCart(m)}
                      className="w-full px-3 py-2 hover:bg-ink-50 flex items-center justify-between text-left"
                    >
                      <div className="flex items-center gap-2">
                        <Pill className="w-4 h-4 text-ink-500" />
                        <div>
                          <p className="text-sm font-medium">{m.name}</p>
                          <p className="text-xs text-ink-500">SKU: {m.sku}</p>
                        </div>
                      </div>
                      <p className="text-sm font-semibold text-accent-700">{formatVND(m.price)}</p>
                    </button>
                  ))}
                </div>
              )}
                    ...
                    return (
                      <tr key={item.medicine.id}>
                        <td className="py-2">
                          ...
                          <p className="text-xs text-ink-500">{item.medicine.sku}</p>
                        </td>
                        <td className="text-center">
                          <input
                            ...
                            className="w-16 px-2 py-1 text-center border border-ink-300 rounded"
                          />
                          {item.quantity >= 10 && <p className="text-xs text-accent-600 mt-1">-5%</p>}
                        </td>
        ...
        <div className="space-y-4">
          <Card title="Tổng đơn hàng" className="sticky top-20">
            <div className="space-y-3 text-sm">
              ...
              <div className="flex justify-between">
                <span className="text-ink-600">Giảm giá (BR04):</span>
                <span className="text-red-600">-{formatVND(totalDiscount)}</span>
              </div>
              <div className="border-t border-ink-200 pt-3 flex justify-between text-lg">
                <span className="font-bold">Tổng cộng:</span>
                <span className="font-bold text-accent-700">{formatVND(total)}</span>
              </div>
              {totalDiscount > 0 && (
                <p className="text-xs text-accent-600 text-center">🎉 Đã áp dụng giảm giá bulk purchase</p>
              )}
            </div>
            <Button
              onClick={submitOrder}
