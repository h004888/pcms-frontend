  ...
  return (
    <DashboardLayout>
      ...
      {loading ? <LoadingSpinner size="lg" /> : !customer ? <EmptyState title="Không tìm thấy khách hàng" /> : (
        <>
          <div className="mb-6">
            <h1 className="text-2xl font-bold text-ink-900">Lịch sử mua hàng</h1>
            <p className="text-sm text-ink-500 mt-1">Khách hàng: {customer.name}</p>
          </div>

          <Card title="Thông tin khách hàng" className="mb-6">
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              <div>
                <p className="text-xs text-ink-500">Mã KH</p>
                <p className="font-mono font-semibold text-accent-700">{customer.code}</p>
              </div>
              <div>
                <p className="text-xs text-ink-500">Số điện thoại</p>
              ...
              <div>
                <p className="text-xs text-ink-500">Điểm thưởng</p>
                <p className="flex items-center gap-1 text-accent-700 font-bold text-lg">
                ...
                {orders.map((o) => (
                  <li key={o.id} className="py-3 flex items-center justify-between">
                    ...
                    <div className="text-right">
                      <p className="font-semibold text-accent-700">{formatVND(o.total)}</p>
