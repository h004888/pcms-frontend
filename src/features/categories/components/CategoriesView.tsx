  ...
  const columns: Column<Category>[] = [
    { key: 'name', header: 'Tên danh mục', render: (c) => (
      <div className="flex items-center gap-2">
        <TagIcon className="w-4 h-4 text-ink-500" />
        <span className="font-medium">{c.name}</span>
      </div>
    )},
    { key: 'description', header: 'Mô tả', render: (c) => <span className="text-sm text-ink-600">{c.description || '—'}</span> },
    { key: 'createdAt', header: 'Ngày tạo', width: '150px', render: (c) => <span className="text-xs text-ink-500">{formatDateTime(c.createdAt)}</span> },
  ];
  ...
  return (
    <DashboardLayout>
      <ListPage<Category>
        ...
        customActions={(c) => (
          <button onClick={(e) => { e.stopPropagation(); handleDelete(c); }} className="p-1 text-ink-500 hover:text-red-600" title="Xóa">
