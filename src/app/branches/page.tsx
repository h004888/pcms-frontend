...
export default function BranchesPage() {
  ...
  const columns: Column<Branch>[] = [
    ...
    { key: 'address', header: 'Địa chỉ', render: (b) => <span className="text-sm text-gray-600">{b.address}</span> },
    { key: 'phone', header: 'SĐT', width: '130px' },
    {
      key: 'status', header: 'Trạng thái', width: '110px',
      render: (b) => <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${getStatusColor(b.status)}`}>{b.status}</span>,
    },
    { key: 'updatedAt', header: 'Cập nhật', width: '150px', render: (b) => <span className="text-xs text-gray-500">{formatDateTime(b.updatedAt)}</span> },
  ];
