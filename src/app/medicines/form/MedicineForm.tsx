...
export function MedicineForm({ open, onClose, item, onSuccess }: {
  ...
  const { register, handleSubmit, formState: { errors, isSubmitting }, reset } = useForm({
    defaultValues: { sku: '', name: '', categoryId: '', price: 0, unit: 'box', prescriptionRequired: false, status: 'ACTIVE' as 'ACTIVE' | 'INACTIVE' },
  });

  useEffect(() => {
    if (item) {
      reset({
        sku: item.sku, name: item.name, categoryId: item.categoryId, price: item.price,
        unit: item.unit, prescriptionRequired: item.prescriptionRequired, status: item.status,
      });
    } else {
      reset({ sku: '', name: '', categoryId: '', price: 0, unit: 'box', prescriptionRequired: false, status: 'ACTIVE' });
    }
  }, [item, reset, open]);
