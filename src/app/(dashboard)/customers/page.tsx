...
export default function CustomersPage() {
  ...
  const columns: Column<Customer>[] = [
    { key: 'code', header: 'Mã KH', width: '130px', render: (c) => <span className="font-mono font-semibold text-accent-700">{c.code}</span> },
    { key: 'name', header: 'Họ tên', render: (c) => <p className="font-medium">{c.name}</p> },
    { key: 'phone', header: 'SĐT', width: '130px' },
    { key: 'email', header: 'Email', render: (c) => <span className="text-sm text-ink-600">{c.email || '—'}</span> },
    { key: 'points', header: 'Điểm thưởng', width: '120px', render: (c) => (
      <div className="flex items-center gap-1">
        <Star className="w-3 h-3 text-yellow-500 fill-yellow-500" />
        <span className="font-semibold text-accent-700">{c.points}</span>
      </div>
    )},
    { key: 'createdAt', header: 'Ngày tạo', width: '130px', render: (c) => <span className="text-xs text-ink-500">{formatDateTime(c.createdAt)}</span> },
  ];
      ...
      <ListPage<Customer>
        ...
        customActions={(c) => (
          <Link
            ...
            className="p-1 text-ink-500 hover:text-ink-500"
