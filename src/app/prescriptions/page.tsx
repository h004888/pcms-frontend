...
export default function PrescriptionsPage() {
  ...
  return (
    <DashboardLayout>
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900">Đơn thuốc (Prescription)</h1>
        <p className="text-sm text-gray-500 mt-1">UC12 - Dược sĩ kê đơn · FR12.1-FR12.4</p>
      </div>
        ...
        <Card title="Đơn thuốc gần đây">
          {recent.length === 0 ? <EmptyState title="Chưa có đơn thuốc" /> : (
            <ul className="space-y-2">
              {recent.map((p) => (
                <li key={p.id} className="flex items-start gap-3 p-3 rounded-md hover:bg-gray-50">
                  <FileText className="w-5 h-5 text-primary-600 mt-0.5" />
                  <div className="flex-1">
                    <p className="font-medium text-sm">{p.code}</p>
                    <p className="text-xs text-gray-500 line-clamp-1">{p.diagnosis}</p>
                  </div>
                  <span className="text-xs px-2 py-0.5 bg-green-100 text-green-800 rounded">{p.status}</span>
                </li>
              ))}
            </ul>
          )}
