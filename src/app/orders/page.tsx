...
export default function OrdersPage() {
  ...
  const columns: Column<Order>[] = [
    { key: 'orderNumber', header: 'Mã đơn', width: '170px', render: (o) => <span className="font-mono font-semibold text-primary-700">{o.orderNumber}</span> },
    { key: 'createdAt', header: 'Ngày tạo', width: '150px', render: (o) => <span className="text-xs text-gray-500">{formatDateTime(o.createdAt)}</span> },
    ...
    { key: 'total', header: 'Tổng', width: '140px', align: 'right', render: (o) => <span className="font-semibold text-medical-700">{formatVND(o.total)}</span> },
      ...
      <ListPage<Order>
        ...
        customActions={(o) => (
          <div className="flex items-center justify-end gap-1">
            <Link href={`/orders/${o.id}`} onClick={(e) => e.stopPropagation()} className="p-1 text-gray-500 hover:text-primary-600" title="Xem chi tiết">
              <Eye className="w-4 h-4" />
            </Link>
            {o.status === 'PENDING_PAYMENT' && (
              <button onClick={(e) => { e.stopPropagation(); handleCancel(o); }} className="p-1 text-gray-500 hover:text-red-600" title="Hủy đơn">
